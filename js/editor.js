(function(){
  ...
  pn.removeButton('options', 'gjs-open-import-template');
  ...
  editor.Commands.add('app-save', {
    async run(ed){
      ...
      await window.StorageProvider.save({ mjml, css, json, ts: Date.now() });
  ...
  bm.add('hero', {
    ...
    content: `
      <mj-section background-color="#ffffff" padding="0 16px 24px">
        <mj-column>
          <mj-image src="https://via.placeholder.com/800x300" alt="Hero"></mj-image>
          <mj-text font-size="22px" font-weight="700" padding-top="16px">Headline</mj-text>
          <mj-text>Intro paragraph goes here.</mj-text>
          <mj-button href="#" background-color="\${Theme.brandColor}">Call to Action</mj-button>
        </mj-column>
      </mj-section>
    `
  });
  ...
  setInterval(async ()=>{
    ...
    await window.StorageProvider.save(json);
  }, 10000);

  // Load or default template
  (async function boot(){
  ...
  editor.on('asset:upload:add', async (files) => {
    ...
    dataUrls.forEach(url => editor.AssetManager.add({ src: url }));
  });
})();
