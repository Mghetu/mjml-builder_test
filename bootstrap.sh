...
mkdir -p scripts vendor/grapesjs vendor/mjml-browser public
...
echo "▶ Writing server.py (auto-picks a free port)…"
cat > server.py <<'EOF'
import os
import socket
...
{
  "name": "mjml-builder_test",
  "version": "1.0.0",
  "description": "GrapesJS MJML Email Editor (Codespaces-ready)",
  "private": true,
  "scripts": {
    "build:vendor": "bash scripts/build_vendor.sh",
    "serve": "python3 server.py",
    "dev": "npm run build:vendor && npm run serve"
  },
  "dependencies": {
    "grapesjs": "^0.22.13",
    "grapesjs-mjml": "^1.0.7",
...
echo "▶ Writing scripts/build_vendor.sh…"
cat > scripts/build_vendor.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
if [ -f node_modules/grapesjs-mjml/dist/grapesjs-mjml.min.js ]; then
  ...
  npx esbuild "$ENTRY" --bundle --format=iife --outfile=vendor/grapesjs-mjml.min.js
...
EOF
chmod +x scripts/build_vendor.sh
...
<head>
  <meta charset="utf-8" />
  <title>MJML Email Editor — GrapesJS</title>
...
<body>
  <div id="topbar">
    <span id="title">📬 MJML Email Editor</span>
  ...
  <!-- MJML in the DOM; plugin parses this with fromElement:true -->
  <div id="gjs">
    <mjml>
      <mj-head>
        <mj-attributes>
      ...
      <mj-body background-color="#f3f4f6">
        ...
        <mj-section background-color="#ffffff" padding="24px">
          <mj-column>
            <mj-text font-size="22px" font-weight="700">Hello 👋</mj-text>
            <mj-text>Build MJML emails visually with GrapesJS. Export compiled HTML when you’re done.</mj-text>
            <mj-button background-color="#111827" color="#ffffff" padding="12px 20px" href="https://grapesjs.com">Open docs</mj-button>
          </mj-column>
        </mj-section>
        <mj-section>
          <mj-column>
            <mj-text align="center" color="#6b7280" font-size="12px">You’re receiving this because you opted in.</mj-text>
          </mj-column>
        </mj-section>
      </mj-body>
    </mjml>
  ...
  <script>
    ...
    (function boot(){
      ...
      const editor = grapesjs.init({
        ...
        plugins: ['grapesjs-mjml'],       // use plugin by id (UMD registers it)
        pluginsOpts: {
          'grapesjs-mjml': {
            resetBlocks: true,
            resetDevices: true,
      ...
      $('#btn-export-html').addEventListener('click', () => {
        try {
          ...
          editor.Modal.open({
            title: 'Exported HTML',
            content: \`<textarea style="width:100%;height:60vh;font:12px/1.4 ui-monospace,Menlo,Consolas,monospace;background:#0f1115;color:#e7ecf3;border:1px solid #2a3246;border-radius:8px;padding:12px;">\${full.replace(/&/g,'&amp;').replace(/</g,'&lt;')}</textarea>\`
          });
        } catch (e) {
          console.error(e); setStatus('Export failed — see console', false);
        }
      });
      ...
      $('#btn-download-html').addEventListener('click', () => {
        try {
          ...
          console.error(e); setStatus('Download failed — see console', false);
        }
      });
      ...
      $('#btn-reset').addEventListener('click', () => {
        try {
          ...
          setStatus('Reset failed — see console', false);
        }
      });
    })();
  </script>
...
echo "✅ Done. Start the server with:  npm run serve"
